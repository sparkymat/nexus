...
func DoRegister() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		_, err := model.FetchUserByEmail(c.Request().Context(), email)
		if err == nil {
		...
		_, err = model.CreateUser(c.Request().Context(), name, email, string(encryptedPasswordBytes))
		if err != nil {
...
func renderRegistrationPage(c echo.Context, name string, email string, errorMessage string) error {
	...
	pageHTML := view.Register(csrfToken, name, email, errorMessage)
	document := view.Layout("nexus | register", csrfToken, pageHTML)
